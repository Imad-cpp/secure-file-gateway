...
Operational hardening adds two further rules:

> **Security correlation identifiers are generated by the server, not trusted from client input.**

> **Audit and readiness surfaces must not become new disclosure channels for secrets, signed URLs, object keys or infrastructure details.**
...
health/readiness + deleted-object reconciliation
```
...
- readiness uses only the scanner `PING` command;
...
- audit metadata sanitizer drops URL/signature-like keys;
...
- `files:reconcile-deleted` retries only object keys still referenced by `DELETED` rows.

**Known boundary:** PostgreSQL and object storage do not share a transaction. The implemented reconciliation is targeted, not a generic bucket-wide orphan sweep.

### 15. Credential / token leakage

**Threat:** tokens, signed capabilities or storage credentials appear in source code, logs, audit rows or CI output.
...
- audit sanitizer recursively drops token/password/secret/credential/signature/URL/object-key/body/payload/content-like keys.
...
- audit events use bounded actor/action/target/outcome/request-correlation metadata;
- audit rows never intentionally contain file bodies, bearer tokens, storage credentials, signed URLs, raw Authorization headers or private object keys;
- sanitizer recursively drops high-risk key classes before persistence;
- string metadata is length-bounded;
- normal-user APIs do not expose audit rows;
- an audit-write failure logs only action, outcome and request ID.

**Known boundary:** audit persistence is best-effort. It is not a transactional forensic ledger and does not guarantee an event survives every database outage or partial failure.

### 17. Correlation-ID spoofing or log injection

**Threat:** a client supplies a crafted request ID to confuse incident correlation or inject attacker-controlled text into logs/audit records.

**Controls:**

- a new UUID is generated server-side for every HTTP request;
- inbound `X-Request-ID` is ignored rather than trusted;
- the generated ID is returned on normal and rendered error responses;
- audit events reuse only the validated server-generated UUID;
- tests verify spoofed inbound IDs are replaced.

### 18. Readiness information disclosure

**Threat:** a public health endpoint reveals which internal dependency failed, credentials, bucket names, scanner topology or raw exceptions.

**Controls:**

- readiness checks dependencies internally;
- public output contains only aggregate `ready` / `not_ready`;
- dependency names, credentials, bucket names and raw failures are not returned;
- any failed dependency causes HTTP `503`.

### 19. Unsafe cleanup reconciliation

**Threat:** an automated cleanup process deletes unrelated or still-active objects.

**Controls:**

- the reconciliation command selects only rows already in terminal `DELETED` state;
- it operates only on object keys still referenced by those rows;
- successful keys are cleared only after storage deletion succeeds;
- non-deleted rows are explicitly covered by tests;
- V1 does not run a generic bucket-wide orphan deletion sweep.
...
- sanitized internal audit metadata;
- aggregate-only public readiness;
- targeted reconciliation only;
...
- client-supplied request IDs cannot override server correlation;
- error responses retain server-generated correlation IDs;
- audit metadata sanitizer excludes known sensitive key classes;
- failed-login audit does not store submitted credentials;
- readiness fails closed without exposing dependency details;
- deleted-object reconciliation never touches non-deleted rows.

Current CI covers the implemented identity, ingestion, scanning, controlled-delivery, deletion, request-correlation, audit-sanitization, readiness-response and targeted-reconciliation layers. The permanent gate also enforces a committed dependency lock, Larastan/PHPStan level 5 without a baseline, OpenAPI route-drift validation, Composer dependency audit and full-history Gitleaks scanning. Its real-container job boots PostgreSQL, Redis, both MinIO storage zones, ClamAV and Laravel, requires aggregate readiness, starts the Redis-backed scan worker, then exercises clean and runtime-generated EICAR uploads through the real HTTP/application path. The clean fixture must become `AVAILABLE` and stream byte-identical signed content before deletion; the EICAR fixture must become `REJECTED` and remain non-downloadable. This is real-engine integration evidence, not a claim of malware-detection completeness, arbitrary-file safety or production operational maturity.

## Vulnerability reporting

`SECURITY.md` now defines the private reporting route, pre-V1 supported-version boundary, responsible-testing expectations and disclosure process.
...
The current V1 also does not claim a transactional audit ledger, a generic bucket-wide orphan sweeper, production monitoring/incident response or production deployment maturity.
